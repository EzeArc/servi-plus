...
export const CategorySystem = ({ children, currentLocale = "es" }) => {
  ...
  const { categoryName = "", provider } = pageInfo ?? {};
  ...
  if (categoryCode === MAX_CATEGORIES) {
    return (
      <VoteFinal
        provider={providerByCategory}
        votes={votes}
        setCategory={setCategoryCode}
        categoryNames={providerByCategory.map(
  ...
  return (
    <>
      ...
      <ul
        ref={containerRef}
        class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-2 px-8 lg:px-24 xl:px-0 min-h-[343px]"
      >
        {provider?.map((candidate, index) => {
          ...
          return (
            ...
            >
              ...
              <button
                ...
                `}
                onClick={() => setVotesCategory({ provider: id })}
              >
                {voteIndex >= 0 && (
...
const CategoryTitle = ({ children, isChanging }) => {
  return (
